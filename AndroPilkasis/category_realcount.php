...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
  ...
  if($auth_condition){
    ...
    $result["category_filter_name"] = $category_filter;

    // count total user available
    $count_user_sql = "SELECT COUNT(1) AS count_user FROM user_pilkasis WHERE kelas LIKE '$category_filter'";
    ...
    $count_golput_sql = "SELECT COUNT(1) AS count_golput FROM user_pilkasis WHERE pilih_ketua_id LIKE '0' AND kelas LIKE '$category_filter'";
    ...
    while($row_kandidat = mysqli_fetch_array($kandidat_query)){
      ...
      $count_selected_sql = "SELECT COUNT(1) AS count_selected FROM user_pilkasis WHERE pilih_ketua_id LIKE '$kandidat_id' AND kelas LIKE '$category_filter'";
      $count_selected_query = mysqli_query($con, $count_selected_sql);
      $count_selected = mysqli_fetch_array($count_selected_query);
